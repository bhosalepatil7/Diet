...
Partial Class Master_login
    ...
    Protected Sub Page_Error(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Error
        ...
        acs.WriteError(sLastError, sStackTrace, sUser, PhyFilePath, "Geeta Nutri Heal")
    End Sub
    ...
    Protected Sub Login_Authenticate(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.AuthenticateEventArgs) Handles Login.Authenticate
        ...
        'Added by sonali
        cmd.CommandText = "SELECT COUNT(*) FROM ADMIN_MAPS_USER_MASTER WHERE UPPER(USER_NAME)=UPPER(@USERNAME) AND (VALID_UPTO < CAST(dateadd(minute,330,getutcdate()) AS date))"
        cmd.Connection = conn
        cmd.Parameters.Add(prmUser)
        prmUser.Value = Login.UserName
        If conn.State = ConnectionState.Closed Then conn.Open()
        ...
        cmd.CommandText = "INSERT INTO SESSION_TRACKING_LOG ( indexno,USERNAME, HOSTNAME, LOGINAT) VALUES (" & _IndexNumber & ",UPPER(@USERNAME),@HOST,dateadd(minute,330,getutcdate()))"
        cmd.Parameters.Add(prmUser)
        ...
        If CHECKOUT() Then
            ...
            Dim cmd As New SqlCommand("insert into log values(@UserName,'" & hostname & "',dateadd(minute,330,getutcdate()),'','" & Session.SessionID & "','')", conn)
    ...
    Private Sub THIRTY_DAYS()
        ...
        Dim SQL1 As String = "SELECT DATEDIFF(DAY, dateadd(minute,330,getutcdate()), DATEADD(day, 30, PASSWORD_DATE)) AS dt_cnt FROM ADMIN_MAPS_USER_MASTER WHERE UPPER(USER_NAME)= UPPER(@USERNAME) "
