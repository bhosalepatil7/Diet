...
#End Region
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Response.ExpiresAbsolute = DateTime.UtcNow.AddHours(5.5).AddDays(-1.0)
        Response.Expires = 0
    ...
    Public Sub SaveNewUser()
        ...
        If ddlGROUpID.SelectedValue.Trim().ToUpper().Equals("RECEPTIONIST") Then
            If IsGroupValid("val2") = False Then
                Tell.text("Select Doctor Validation Failed", Me)
                Return
            End If
        ElseIf ddlGROUpID.SelectedValue.Trim().ToUpper().Equals("RESEARCH DOCTOR") Then
            If IsGroupValid("val3") = False Then
                Tell.text("Select Superior Validation Failed", Me)
                Return
            End If
        End If
        ...
        If Session("EDITFLAG") = "TRUE" Then
            ...
            cmdInsert.CommandText = "INSERT INTO ADMIN_MAPS_USER_MASTER ( USER_ID ,  USER_NAME, USER_PASSWORD, ACCOUNT_STATUS, GROUP_ID, BLOCKED, DISABLED, ENROL_DATE, PASSWORD_DATE, ADDED_BY, UPDATED_BY ,VALID_FROM, VALID_UPTO , USER_FULL_NAME,country,state,city,PARENT_ID,gender) VALUES ( @USER_ID, upper(@USER_NAME),@USER_PASSWORD, @ACCOUNT_STATUS, @GROUP_ID, 'N', 'N', dateadd(minute,330,getutcdate()), dateadd(minute,330,getutcdate()), @ADDED_BY, @UPDATED_BY ,@VALID_FROM, @VALID_UPTO, @USER_FULL_NAME,'India','Maharashtra','Pune',@parent,@gender)"
            ...
            Dim gender As New SqlParameter("@gender", SqlDbType.NVarChar, 6)
            Dim parent As New SqlParameter("@parent", SqlDbType.NVarChar, 100)
            ...
            cmdInsert.Parameters.Add(parent)
            ...
            gender.Value = ddlGender.SelectedValue
            If ddlGROUpID.SelectedValue.Trim().ToUpper().Equals("RECEPTIONIST") Then
                parent.Value = ddlDoctor.SelectedValue.Trim()
            ElseIf ddlGROUpID.SelectedValue.Trim().ToUpper().Equals("RESEARCH DOCTOR") Then
                parent.Value = ddlSuperior.SelectedValue.Trim()
            Else
                parent.Value = ""
            End If
            ...
            If SRE = 0 Then
                ...
                Dim CMDX As New SqlCommand("INSERT INTO ADMIN_MAPS_PASSWORD_HISTORY VALUES('1',dateadd(minute,330,getutcdate()),EncryptByKey(Key_GUID('Key_Encript'),''),EncryptByKey(Key_GUID('Key_Encript'),@NEW_PASSWORD),@USERID)", conn)
    ...
    Private Function returngroupId(ByVal groupname As String) As Integer
        ...
        Dim str = "select group_id from admin_maps_group_master where upper(group_name) = upper(@grpname) "
    ...
    Private Sub Page_Error(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Error
        ...
        acs.WriteError(sLastError, sStackTrace, sUser, PhyFilePath, "GNH AddNewUsers")
    End Sub
    ...
    Protected Sub ddlGROUpID_SelectedIndexChanged1(sender As Object, e As EventArgs)
        If ddlGROUpID.SelectedValue.Trim().ToUpper().Equals("RECEPTIONIST") Then
            bindDDL(1)
    ...
    End Sub
    Private Function bindDDL(ByVal choice As Int32)
        ...
        If choice = 1 Then
            cmd.Connection = conn
            cmd.CommandType = CommandType.Text
            cmd.CommandText = "SELECT '- -Select- -' user_name union all select user_name from admin_maps_user_master where upper(rtrim(ltrim(group_id)))='RESEARCH DOCTOR'"
            da.SelectCommand = cmd
            da.Fill(ds)
            ddlDoctor.DataTextField = "user_name"
            ddlDoctor.DataSource = ds
            ddlDoctor.DataBind()
        ElseIf choice = 2 Then
            cmd.Connection = conn
            cmd.CommandType = CommandType.Text
            cmd.CommandText = "SELECT '- -Select- -' user_name union all select user_name from admin_maps_user_master where upper(rtrim(ltrim(group_id)))='SUPERIOR DOCTOR'"
            da.SelectCommand = cmd
            da.Fill(ds)
            ddlSuperior.DataTextField = "user_name"
            ddlSuperior.DataValueField = "user_name"
